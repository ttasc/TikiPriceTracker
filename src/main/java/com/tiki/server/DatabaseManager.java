...
public class DatabaseManager {
	...
	public List<Product> getTrackedProducts() {
		...
		// Lấy thông tin sản phẩm và giá mới nhất từ bảng lịch sử giá (dùng Subquery)
		String sql = "SELECT p.*, "
		...
		try (Connection conn = DriverManager.getConnection(URL);
				...
				ResultSet rs = stmt.executeQuery(sql)) {
			while (rs.next()) {
				list.add(new Product(rs.getString("id"), rs.getString("name"), rs.getLong("current_price"), // Lấy giá
																											// cuối cùng
																											// đã lưu
						rs.getString("thumbnail"), true // Chắc chắn là đang theo dõi
				));
			}
		} catch (SQLException e) {
			e.printStackTrace();
